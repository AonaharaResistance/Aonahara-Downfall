...
func _ready():
	...
	$MainPanel/MasterTreeContainer/MasterTree.connect(
		"editor_selected", self, "on_master_tree_editor_selected"
	)
	...
	if _scale == 2:
		...
		modifier = "-2"
	$ToolBar/NewTimelineButton.icon = load(
		"res://addons/dialogic/Images/Toolbar/add-timeline" + modifier + ".svg"
	)
	$ToolBar/NewCharactersButton.icon = load(
		"res://addons/dialogic/Images/Toolbar/add-character" + modifier + ".svg"
	)
	$ToolBar/NewValueButton.icon = load(
		"res://addons/dialogic/Images/Toolbar/add-definition" + modifier + ".svg"
	)
	$ToolBar/NewGlossaryEntryButton.icon = load(
		"res://addons/dialogic/Images/Toolbar/add-glossary" + modifier + ".svg"
	)
	$ToolBar/NewThemeButton.icon = load(
		"res://addons/dialogic/Images/Toolbar/add-theme" + modifier + ".svg"
	)
	...
	# Toolbar
	$ToolBar/NewTimelineButton.connect(
		"pressed", $MainPanel/MasterTreeContainer/MasterTree, "new_timeline"
	)
	$ToolBar/NewCharactersButton.connect(
		"pressed", $MainPanel/MasterTreeContainer/MasterTree, "new_character"
	)
	$ToolBar/NewThemeButton.connect(
		"pressed", $MainPanel/MasterTreeContainer/MasterTree, "new_theme"
	)
	$ToolBar/NewValueButton.connect(
		"pressed", $MainPanel/MasterTreeContainer/MasterTree, "new_value_definition"
	)
	$ToolBar/NewGlossaryEntryButton.connect(
		"pressed", $MainPanel/MasterTreeContainer/MasterTree, "new_glossary_entry"
	)
	...
	$ToolBar/Docs.connect(
		"pressed", $MainPanel/MasterTreeContainer/MasterTree, "select_documentation_item", ["/"]
	)
	$ToolBar/FoldTools/ButtonFold.connect("pressed", $MainPanel/TimelineEditor, "fold_all_nodes")
	$ToolBar/FoldTools/ButtonUnfold.connect(
		"pressed", $MainPanel/TimelineEditor, "unfold_all_nodes"
	)
	...
	$MainPanel/MasterTreeContainer/FilterMasterTreeEdit.right_icon = get_icon(
		"Search", "EditorIcons"
	)
...
func popup_remove_confirmation(what):
	...
	if $RemoveConfirmation.is_connected("confirmed", self, "_on_RemoveConfirmation_confirmed"):
		$RemoveConfirmation.disconnect("confirmed", self, "_on_RemoveConfirmation_confirmed")
...
func _on_RemoveFolderConfirmation_confirmed():
	var item_path = $MainPanel/MasterTreeContainer/MasterTree.get_item_path(
		$MainPanel/MasterTreeContainer/MasterTree.get_selected()
	)
...
func godot_dialog_connect(who, method_name, signal_name = "file_selected"):
	...
	for test_signal in editor_file_dialog.get_signal_list():
		...
		if editor_file_dialog.is_connected(
			test_signal.name, file_picker_data["node"], file_picker_data["method"]
		):
