...
func load_audio(path: String):
	if not path.empty():
		...
		if event_data.has("audio"):
			event_data["audio"] = "play"
		if event_data.has("background-music"):
...
func _process(_delta):
	...
	if (
		audio_preview.playing
		&& event_data.has("stop_time")
		&& audio_preview.get_playback_position() >= event_data["stop_time"]
	):
		audio_preview.stop()
