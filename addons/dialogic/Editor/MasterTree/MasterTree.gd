...
func _ready():
	...
	documentation_tree.set_metadata(
...
func build_resource_folder(
	parent_folder_item: TreeItem,
	folder_data: Dictionary,
	selected_item: String,
	folder_editor: String,
	resource_type: String
):
	## BUILD ALL THE FOLDER ITEMS (by calling this method for them)
	for folder in folder_data["folders"].keys():
		var folder_item = _add_folder_item(
			parent_folder_item, folder, folder_editor, folder_data["folders"][folder]["metadata"]
		)
		var contains_something = build_resource_folder(
			folder_item, folder_data["folders"][folder], selected_item, folder_editor, resource_type
		)
	...
	for file in folder_data["files"]:
		...
		if (
			(filter_tree_term == "")
			or (filter_tree_term.to_lower() in file_metadata["name"].to_lower())
		):
			_add_resource_item(
				resource_type,
				parent_folder_item,
				file_metadata,
				not selected_item.empty() and file == selected_item
			)
...
func _add_folder_item(
	parent_item: TreeItem, folder_name: String, editor: String, meta_folder_info: Dictionary
):
	# create item
...
func build_definitions(selected_item: String = ""):
	...
	build_resource_folder(
		definitions_tree, structure, selected_item, "Definition Root", "Definition"
	)
...
func _on_item_collapsed(item: TreeItem):
	if (
		filter_tree_term.empty()
		and item != null
		and "Root" in item.get_metadata(0)["editor"]
		and not "Documentation" in item.get_metadata(0)["editor"]
	):
...
func build_documentation(selected_item: String = ""):
	...
	$DocsTreeHelper.build_documentation_tree(
		self,
		documentation_tree,
		{"editor": "Documentation Root", "editable": false},
		{"editor": "Documentation", "editable": false},
		filter_tree_term
	)
...
func create_rmb_context_menus():
	...
	documentation_folder_popup.add_icon_item(
		get_icon("Edit", "EditorIcons"), "Toggle Editing Tools"
	)
...
func _on_ThemePopupMenu_id_pressed(id):
	...
	if id == 1:
		var filename = editor_reference.get_node("MainPanel/MasterTreeContainer/MasterTree").get_selected().get_metadata(
			0
		)["file"]
...
func _on_TimelineRootPopupMenu_id_pressed(id):
	...
	if id == 1:  # add subfolder
		DialogicUtil.add_folder(
			get_item_path(get_selected()), "New Folder " + str(OS.get_unix_time())
		)
		build_timelines()
	...
	if data != null and data is Dictionary and data.has("item_type"):
		...
		if not (data["item_type"] == "folder" and not "Root" in item.get_metadata(0)["editor"]):
			...
			if get_item_folder(item, "").split("/")[0] == data["orig_path"].split("/")[0]:
				...
				if (
					data["item_type"] == "file"
					or (not get_item_folder(item, "").begins_with(data["orig_path"]))
				):
					return true
	return false
...
func drop_data(position, data):
	...
	if data["item_type"] == "folder":
		...
		if "Root" in item.get_metadata(0)["editor"]:
			DialogicUtil.move_folder_to_folder(
				data["orig_path"], get_item_folder(item, data["orig_path"].split("/")[0])
			)
	# dragging a file
		...
		if "Root" in item.get_metadata(0)["editor"]:
			if data.has("file_name"):
				DialogicUtil.move_file_to_folder(
					data["file_name"],
					data["orig_path"],
					get_item_folder(item, data["orig_path"].split("/")[0])
				)
...
func get_drag_data(position):
	...
	if "Root" in item.get_metadata(0)["editor"] and item.get_parent().get_parent():
		...
		if item.get_metadata(0).has("file"):
			...
			return {
				"item_type": "file",
				"orig_path": get_item_folder(item, ""),
				"file_name": item.get_metadata(0)["file"]
			}
