...
static func start(
	...
	if timeline == "":
		if (
			Engine.get_main_loop().has_meta("last_dialog_state")
		...
		## The loaded data isn't complete
			...
			Engine.get_main_loop().has_meta("current_timeline")
			...
			[
				{
					...
					"text":
					(
						"[Dialogic Error] Loading dialog [color=red]"
						+ timeline
						+ "[/color]. It seems like the timeline doesn't exists. Maybe the name is wrong?"
					)
				}
			]
		}
...
static func save(slot_name: String = "", is_autosave = false) -> void:
	...
	var save_data = {"game_state": game_state, "dialog_state": current_dialog_info}
...
static func has_current_dialog_node() -> bool:
	return (
		Engine.get_main_loop().has_meta("latest_dialogic_node")
		and is_instance_valid(Engine.get_main_loop().get_meta("latest_dialogic_node"))
	)
...
static func set_variable(name: String, value):
	...
	if exists == false:
		...
		print(
			(
				"[Dialogic] Warning! the variable ["
				+ name
				+ "] doesn't exists. Create it from the Dialogic editor."
			)
		)
	return value
...
static func _load_from_slot(slot_name: String = "") -> Dictionary:
	Engine.get_main_loop().set_meta(
		"definitions", DialogicResources.get_saved_definitions(slot_name)
	)
...
static func set_variable_from_id(id: String, value: String, operation: String) -> void:
	...
	if target_def != null:
		...
		var is_number = (
			converted_set_value.is_valid_float()
			and converted_target_value.is_valid_float()
		)
		if is_number:
