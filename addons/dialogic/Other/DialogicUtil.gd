...
static func get_theme_list() -> Array:
	...
	for file in DialogicResources.listdir(DialogicResources.get_path("THEME_DIR")):
		if ".cfg" in file:
			...
			themes.append(
				{"file": file, "name": config.get_value("settings", "name", file), "config": config}
			)
	return themes
...
static func get_default_definitions_list() -> Array:
	return DialogicDefinitionsUtil.definitions_json_to_array(
...
static func set_folder_content_recursive(path_array: Array, orig_data: Dictionary, new_data: Dictionary) -> Dictionary:
	if len(path_array) == 1:
		...
		orig_data["folders"][current_folder] = set_folder_content_recursive(
			path_array, orig_data["folders"][current_folder], new_data
		)
	return orig_data
...
static func add_folder(path: String, folder_name: String):
	...
	folder_data["folders"][folder_name] = {
		"folders": {}, "files": [], "metadata": {"color": null, "folded": false}
	}
...
static func update_resource_folder_structure():
	...
	folder_structure["folders"]["Timelines"] = check_folders_section(
		folder_structure["folders"]["Timelines"], timeline_files
	)
	folder_structure["folders"]["Characters"] = check_folders_section(
		folder_structure["folders"]["Characters"], character_files
	)
	folder_structure["folders"]["Themes"] = check_folders_section(
		folder_structure["folders"]["Themes"], theme_files
	)
	folder_structure["folders"]["Definitions"] = check_folders_section(
		folder_structure["folders"]["Definitions"], definition_files
	)
...
static func resource_fixer():
	var update_index = DialogicResources.get_settings_config().get_value(
		"updates", "updatenumber", 0
	)
