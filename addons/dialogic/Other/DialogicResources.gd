...
static func init_dialogic_files() -> void:
	...
	for dir in paths:
		if not directory.dir_exists(paths[dir]):
			...
			if dir == "THEME_DIR":
				directory.copy(
					"res://addons/dialogic/Editor/ThemeEditor/default-theme.cfg",
					str(paths[dir], "/default-theme.cfg")
				)
	# Create empty files
	for f in files:
...
static func remove_file(path: String):
	...
	if _err != OK:
		print(
			"[D] There was an error when deleting file at {filepath}. Error: {error}".format(
...
static func get_config(id: String) -> ConfigFile:
	...
	if id in paths.keys():
		...
		if err != OK:
			print(
				"[Dialogic] Error while opening config file " + paths[id] + ". Error: " + str(err)
			)
	return config
...
static func get_default_definitions() -> Dictionary:
	return load_json(
		get_config_files_paths()["DEFAULT_DEFINITIONS_FILE"], {"variables": [], "glossary": []}
	)
...
static func set_default_definition_glossary(
	id: String, name: String, extra_title: String, extra_text: String, extra_extra: String
):
	# WARNING: For use in the editor only
	var data = get_default_definitions()
	DialogicDefinitionsUtil.set_definition_glossary(
		data, id, name, extra_title, extra_text, extra_extra
	)
	save_default_definitions(data)
...
static func get_saved_definitions(save_name: String = "") -> Dictionary:
	if save_name == "":
		return load_json(
			get_config_files_paths()["DEFINITIONS_DEFAULT_SAVE"], get_default_definitions()
		)
...
static func get_resource_folder_structure() -> Dictionary:
	return load_json(
		get_config_files_paths()["FOLDER_STRUCTURE_FILE"],
		{
			"folders":
			{
				"Timelines":
