...
static func get_translations() -> Dictionary:
	...
	for resource in translations_resources:
		var t: PHashTranslation = load(
			"res://addons/dialogic/Localization/dialogic." + resource + ".translation"
		)
...
static func _get_translation(message) -> String:
	...
	var cases = translations.get(
		locale, translations.get(default_fallback, [PHashTranslation.new()])
	)
	for case in cases:
