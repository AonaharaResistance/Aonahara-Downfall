		...
		if visible:
			if get_global_mouse_position().x < get_viewport().size.x * 0.5:
				rect_global_position = (
					...
					- Vector2(0, rect_size.y + (margin * 2))
				)
			else:
				rect_global_position = (
					get_global_mouse_position()
...
func load_theme(theme):
	...
	$VBoxContainer/Title.set(
		"custom_colors/default_color", theme.get_value("definitions", "title_color", "#ffffffff")
	)
	...
	$VBoxContainer/Content.set(
		"custom_colors/default_color", theme.get_value("definitions", "text_color", "#c1c1c1")
	)
	...
	$VBoxContainer/Extra.set(
		"custom_colors/default_color", theme.get_value("definitions", "extra_color", "#c1c1c1")
	)
