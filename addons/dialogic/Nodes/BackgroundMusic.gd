...
func crossfade_to(path: String, audio_bus: String, volume: float, fade_length: float) -> void:
	...
	$Tween.interpolate_property(
		fade_out_track, "volume_db", null, -60, fade_length, Tween.TRANS_EXPO
	)
	$Tween.interpolate_property(
		fade_in_track, "volume_db", -60, volume, fade_length, Tween.TRANS_EXPO
	)
	$Tween.start()
...
func get_current_info():
	if current_track != null:
		return {
			"file": current_path, "volume": current_track.volume_db, "audio_bus": current_track.bus
		}
	return null
