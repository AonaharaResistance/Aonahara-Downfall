...
func load_theme(theme: ConfigFile):
	# Text
	var theme_font = DialogicUtil.path_fixer_load(
		theme.get_value(
	...
	# Backgrounds
	$TextureRect.texture = DialogicUtil.path_fixer_load(
		theme.get_value(
			"background",
			"image",
			"res://addons/dialogic/Example Assets/backgrounds/background-2.png"
		)
	)
	...
	next_indicator.texture = DialogicUtil.path_fixer_load(
		theme.get_value(
			"next_indicator",
			"image",
	...
	next_indicator.rect_position = (
		theme.get_value("box", "size", Vector2(910, 167))
		- (next_indicator.texture.get_size() * indicator_scale)
	)
	...
	$NameLabel/TextureRect.texture = DialogicUtil.path_fixer_load(
		theme.get_value(
	...
	if theme.get_value("name", "shadow_visible", true):
		name_label.set(
			"custom_colors/font_color_shadow", Color(theme.get_value("name", "shadow", "#9e000000"))
		)
	...
	if theme.get_value("name", "modulation", false) == true:
		$NameLabel/TextureRect.modulate = Color(
	...
	if get_parent().has_node("fade_in_tween_show_time") == false:
		if _finished == false:
			...
			if text_label.visible_characters > text_label.get_total_character_count():
				...
				text_label.visible_characters > 0
				and text_label.text[text_label.visible_characters - 1] != " "
			):
...
func align_name_label():
	...
	if name_label_position == 0:
		...
		name_label.rect_global_position.x = (
