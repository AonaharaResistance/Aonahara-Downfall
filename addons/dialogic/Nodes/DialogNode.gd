...
func _ready():
	...
	$TextBubble/Control/RichTextLabel.connect(
		"meta_hover_started", self, "_on_RichTextLabel_meta_hover_started"
	)
	$TextBubble/Control/RichTextLabel.connect(
		"meta_hover_ended", self, "_on_RichTextLabel_meta_hover_ended"
	)
			...
			if dir.current_is_dir() and not file_name in [".", ".."]:
				...
				if event:
					custom_events[event.event_data["event_id"]] = {
						"event_script":
						path.plus_file(file_name).plus_file(
							"event_" + event.event_data["event_id"] + ".gd"
						),
...
func resize_main():
	...
	var gap_v = current_theme.get_value(
		"box", "box_margin_v", current_theme.get_value("box", "bottom_gap", 40)
	)
	var gap_h = current_theme.get_value(
		"box", "box_margin_h", current_theme.get_value("box", "bottom_gap", 40)
	)
...
func load_theme(filename):
	...
	# Box size
	call_deferred(
		"deferred_resize", $TextBubble.rect_size, theme.get_value("box", "size", Vector2(910, 167))
	)
...
func load_audio(theme):
	...
	for audio_node in $FX/Audio.get_children():
		...
		if file_system.dir_exists(audio_data[name].path):
			...
			file_system.file_exists(audio_data[name].path)
			or file_system.file_exists(audio_data[name].path + ".import")
		):
...
func parse_characters(dialog_script):
	...
	for event in dialog_script["events"]:
		...
		if event.get("event_id") in ["dialogic_001", "dialogic_010"]:
			var text: String = event.get(
				{"dialogic_001": "text", "dialogic_010": "question"}[event.get("event_id")], ""
			)
			...
			for character in characters:
				...
				var regex_thing = (
					str(char_names).replace("[", "(").replace("]", ")").replace(", ", "|")
					+ "\\b"
				)
				...
				for result in regex.search_all(text):
					text = text.insert(
						result.get_start() + ((9 + 8 + 8) * counter),
						"[color=#" + character["color"].to_html() + "]"
					)
					text = text.insert(
						result.get_end() + 9 + 8 + ((9 + 8 + 8) * counter), "[/color]"
					)
...
func _insert_glossary_definitions(text: String):
	...
	for d in definitions["glossary"]:
		final_text = final_text.replace(
			d["name"],
			"[url=" + d["id"] + "]" + "[color=" + color + "]" + d["name"] + "[/color]" + "[/url]"
		)
	return final_text
	...
	if not Engine.is_editor_hint() and event.is_action_pressed(input_next) and not waiting:
		if not $TextBubble.is_finished():
			...
			if (
				...
				and waiting_for_input == false
				and while_dialog_animation == false
			):
...
func _on_text_completed():
	...
	if current_event.has("options"):
		var waiting_until_options_enabled = float(
			settings.get_value("input", "delay_after_options", 0.1)
		)
...
func event_handler(event: Dictionary):
	...
	match event["event_id"]:
		...
		"dialogic_012":
			...
			var condition_met = (
				def_value != null
				and _compare_definitions(def_value, event["value"], event["condition"])
			)
				...
				value = str(
					(
						(
							...
							% int(
								(
									event.get("random_upper_limit", 100)
									- event.get("random_lower_limit", 0)
								)
							)
						)
						+ event.get("random_lower_limit", 0)
		...
		"dialogic_025":
			...
			if event["glossary_id"]:
				Dialogic.set_glossary_from_id(
					event["glossary_id"], event["title"], event["text"], event["extra"]
				)
			_load_next_event()
		# Save event
		"dialogic_026":
			...
			if event.get("use_default_slot", true) or custom_slot == "":
				...
				if custom_slot.begins_with("[") and custom_slot.ends_with("]"):
					...
					if not saved:
						print(
							(
								"[D] Tried to access value definition '"
								+ custom_slot
								+ "' for saving, but it didn't exist."
							)
						)
				else:
		...
		"dialogic_031":
			...
			if (
				event["background-music"] == "play"
				and "file" in event.keys()
				and not event["file"].empty()
			):
				$FX/BackgroundMusic.crossfade_to(
					event["file"],
					event.get("audio_bus", "Master"),
					event.get("volume", 0),
					event.get("fade_length", 1)
				)
			else:
...
func update_name(character) -> void:
	if character.has("name"):
		...
		$TextBubble.update_name(
			parsed_name,
			character.get("color", Color.white),
			current_theme.get_value("name", "auto_color", true)
		)
	else:
		$TextBubble.update_name("")
...
func update_text(text: String) -> String:
	if (
		settings.has_section_key("dialog", "translations")
...
func add_choice_button(option: Dictionary):
	...
	if use_native_choice_button() or use_custom_choice_button():
		button_container.set(
			"custom_constants/separation", current_theme.get_value("buttons", "gap", 20)
		)
...
func _should_add_choice_button(option: Dictionary):
	if not option["definition"].empty():
		...
		return (
			def_value != null
			and _compare_definitions(def_value, option["value"], option["condition"])
		)
	else:
		return true
...
func use_custom_choice_button():
	return (
		current_theme.get_value("buttons", "use_custom", false)
		and not current_theme.get_value("buttons", "custom_path", "").empty()
	)
...
func button_style_setter(section, data, button, theme):
	...
	if data[2]:
		...
		style_box.set(
			"texture",
			DialogicUtil.path_fixer_load("res://addons/dialogic/Images/Plugin/white-texture.png")
		)
	...
	for button in button_container.get_children():
		if button.is_connected("pressed", self, "answer_question") == false:
			button.connect(
				...
				self,
				"answer_question",
				[button, button.get_meta("event_idx"), button.get_meta("question_idx")]
			)
...
func resume_state_from_info(state_info):
	...
	if state_info["background_music"] != null:
		$FX/BackgroundMusic.crossfade_to(
			state_info["background_music"]["file"],
			state_info["background_music"]["audio_bus"],
			state_info["background_music"]["volume"],
			1
		)
