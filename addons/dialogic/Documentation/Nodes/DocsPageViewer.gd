...
func load_page(page_path: String, section: String = ""):
	...
	Content.set(
		"custom_fonts/bold_italics_font/size", int(base_size * get_constant("scale", "Editor"))
	)
	...
	MarkdownParser.set_accent_colors(
		get_color("accent_color", "Editor"), get_color("disabled_font_color", "Editor")
	)
	...
	current_headings = (
		MarkdownParser.heading1s
		+ MarkdownParser.heading2s
		+ MarkdownParser.heading3s
		+ MarkdownParser.heading4s
		+ MarkdownParser.heading5s
	)
...
func scroll_to_section(title):
	...
	for heading in current_headings:
		if (
			(heading.to_lower().strip_edges().replace(" ", "-") == title.replace("#", ""))
			or (heading.to_lower().strip_edges() == title.to_lower().strip_edges())
		):
...
func _on_meta_clicked(meta):
	...
	## Else send a signal that the pluginmaker has to interpret
		...
		if link.begins_with("."):
			link = (
				current_path.trim_suffix(current_path.get_file()).trim_suffix("/")
				+ link.trim_prefix(".")
			)
