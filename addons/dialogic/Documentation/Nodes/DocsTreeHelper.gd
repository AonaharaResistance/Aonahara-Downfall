...
func build_documentation_tree(
	tree: Tree,
	root_item: TreeItem = null,
	def_folder_info: Dictionary = {},
	def_page_info: Dictionary = {},
	filter_term: String = ""
):
...
func _build_documentation_tree(
	tree: Tree,
	root_item: TreeItem = null,
	def_folder_info: Dictionary = {},
	def_page_info: Dictionary = {},
	...
	#print(doc_structure)
	create_doc_tree(
		tree, documentation_tree, def_folder_info, def_page_info, doc_structure, filter_term
	)
...
func create_doc_tree(tree, parent_item, def_folder_info, def_page_info, doc_structure, filter_term):
	for key in doc_structure.keys():
		...
		if typeof(doc_structure[key]) == TYPE_DICTIONARY:
			var folder_item = _add_documentation_folder(
				tree, parent_item, {"name": key.get_file(), "path": key}, def_folder_info
			)
			create_doc_tree(
				tree, folder_item, def_folder_info, def_page_info, doc_structure[key], filter_term
			)
			if not filter_term:
			...
			for file in doc_structure[key]:
				if use_folder_files and file.trim_suffix(".md") in doc_structure.keys():
					...
					if (
						...
						or (
							filter_term
							and filter_term.to_lower() in get_title(file, "").to_lower()
						)
					):
						_add_documentation_page(
							tree,
							parent_item,
							{"name": file.get_file().trim_suffix(".md"), "path": file},
							def_page_info
						)
