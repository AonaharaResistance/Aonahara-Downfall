...
func parse(content: String, file_path: String = "", docs_path: String = ""):
	...
	if result:
		for res in result:
			parsed_text = parsed_text.replace(
				"**" + res.get_string("boldtext") + "**",
				"[b]" + res.get_string("boldtext") + "[/b]"
			)
	...
	if result:
		for res in result:
			parsed_text = parsed_text.replace(
				"__" + res.get_string("underlinetext") + "__",
				"[u]" + res.get_string("underlinetext") + "[/u]"
			)
	...
	if result:
		for res in result:
			parsed_text = parsed_text.replace(
				"*" + res.get_string("italictext") + "*",
				"[i]" + res.get_string("italictext") + "[/i]"
			)
...
#		content = content.replace("*"+italic+"*",)
	...
	if result:
		for res in result:
			parsed_text = parsed_text.replace(
				"~~" + res.get_string("strikedtext") + "~~",
				"[s]" + res.get_string("strikedtext") + "[/s]"
			)
	...
	if result:
		for res in result:
			parsed_text = parsed_text.replace(
				"`" + res.get_string("coded") + "`",
				(
					"[color=#"
					+ accent_color.lightened(0.6).to_html()
					+ "][code]"
					+ res.get_string("coded")
					+ "[/code][/color]"
				)
	...
	if result:
		for res in result:
			...
			if parsed_text.find(symbol + " " + element):
				parsed_text = parsed_text.replace(
					symbol + " " + element, "[indent]" + symbol + " " + element + "[/indent]"
				)
	...
	if result:
		for res in result:
			...
			parsed_text = parsed_text.replace(
				...
				(
					"[color=#"
					+ accent_color.lightened(0.2).to_html()
					+ "][font="
					+ heading1_font
					+ "]"
					+ heading.strip_edges()
	...
	if result:
		for res in result:
			...
			parsed_text = parsed_text.replace(
				...
				(
					"[color=#"
					+ accent_color.lightened(0.5).to_html()
					+ "][font="
					+ heading2_font
					+ "]"
					+ heading.strip_edges()
	...
	if result:
		for res in result:
			...
			parsed_text = parsed_text.replace(
				...
				(
					"[color=#"
					+ accent_color.lightened(0.7).to_html()
					+ "][font="
					+ heading3_font
					+ "]"
					+ heading.strip_edges()
	...
	if result:
		for res in result:
			...
			parsed_text = parsed_text.replace(
				...
				(
					"[color=#"
					+ accent_color.lightened(0.85).to_html()
					+ "][font="
					+ heading4_font
					+ "]"
					+ heading.strip_edges()
	...
	for i in imagenames.size():
		...
		if imagelink_to_use.begins_with(".") and file_path:
			imagelink_to_use = (
				file_path.trim_suffix(file_path.get_file()).trim_suffix("/")
				+ imagelink_to_use.trim_prefix(".")
			)
		parsed_text = parsed_text.replace(
			"![" + imagenames[i] + "](" + imagelinks[i] + ")",
			"[img=" + str(int(700 * editor_scale)) + "]" + imagelink_to_use + "[/img]"
		)
