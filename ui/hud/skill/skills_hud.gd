...
func process_cooldown() -> void:
  ...
  skill_one_cooldown_progress.set_visible(!Party.current_character().skill_one.cooldown_timer.is_stopped())
  skill_two_cooldown_progress.set_visible(!Party.current_character().skill_two.cooldown_timer.is_stopped())

func process_time_label() -> void:
  skill_one_time_left.set_visible(!Party.current_character().skill_one.cooldown_timer.is_stopped())
  skill_two_time_left.set_visible(!Party.current_character().skill_two.cooldown_timer.is_stopped())
  skill_one_time_left.set_text("%.1f" % Party.current_character().skill_one.cooldown_timer.get_time_left())
  skill_two_time_left.set_text("%.1f" % Party.current_character().skill_two.cooldown_timer.get_time_left())
